...
f = open("G:\data-process-data/中国省份的评价.txt", "r", encoding="UTF-8")
data = f.read()
# close the file
f.close()
...
for province in data_dict.keys():
    province_like = data_dict[province]
    #print(province_like)
    data_list.append((province, province_like))
# print(data_list)
...
map=Map()
map.add("厌恶程度打分", data_list, "china")
 # 全局设置
map.set_global_opts(
    title_opts=TitleOpts(title="全国厌恶程度打分"),
    visualmap_opts=VisualMapOpts(
        is_show=True,
        is_piecewise=True,
        pieces=[
            {"min": 0, "max": 0, "label": "喜爱至极", "color": "#E1F0FF"},  # 浅蓝色
            {"min": 1, "max": 1, "label": "非常喜爱", "color": "#C0DFFF"},  # 浅蓝色
            {"min": 2, "max": 2, "label": "还算喜爱", "color": "#A0C4FF"},  # 浅红色
            {"min": 3, "max": 3, "label": "一般喜爱", "color": "#FF6600"},  # 深红色
            {"min": 4, "max": 4, "label": "不咋喜爱", "color": "#FF9999"},  # 橙色
            {"min": 5, "max": 5, "label": "上海厌恶", "color": "#000000"},  # 黑色
        ]

    )
)
# draw the map
map.render("你也不喜欢上海吗.html")
